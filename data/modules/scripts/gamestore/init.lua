...
function parseBuyStoreOffer(playerId, msg)
	...
	if
		...
		or (
			...
			and offer.type ~= GameStore.OfferTypes.OFFER_TYPE_BUNDLES
			and not offer.id
		)
	then
...
function GameStore.processBundlePurchase(player, offer)
	local bundle = offer.bundle
	local looktype
	local addon = offer.bundle.addon or 0
	local sexId = offer.bundle.sexId
	local ItemType = offer.bundle.ItemType
